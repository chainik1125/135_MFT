...
**Consequences.**
- All four boson EBRs are **connected**, glued by a single 4-dim corep at $A$ where $C_{2z}$ acts as $\chi_b(C_{2z})\cdot\mathbb 1$. For bands built on 4a the single-valued analogue of the $8\mathbb Z$ rule is therefore $4\mathbb Z$: a *spinless fermion* on these sites could only form a band insulator at fillings $4\mathbb Z$ (satisfied at 4 by filling the whole EBR).
...
- **This construction:** if the slave-boson mean field converges to a fully gapped, symmetric solution at $\nu=4$ (chargons Mott-gapped, spinons BdG-gapped, trivial PSG), the resulting state is a symmetric gapped insulator that is **long-range entangled**: a deconfined $\mathbb Z_2$ gauge theory in 3+1d. It does *not* contradict the $8\mathbb Z$ rule (it is no band insulator) and does not realize the open "SRE at $\nu=4$" option — it occupies the topologically ordered branch. Ground-state degeneracy $2^3 = 8$ on $T^3$. Pointlike $\mathbb Z_2$ charges: the chargon $b$ (gapped holon/doublon; electric charge $\pm1$, spin-0, $\mathcal T_b^2=+1$) and the spinon $f$ (BdG quasiparticle; charge-neutral, spin-1/2, Kramers) — the two differ by fusion with the physical electron. Flux excitations are **vison loops** (the $\mathbb Z_2$ flux lines surviving the breakdown $U(1)\to\mathbb Z_2$; tension set by the pairing scale), with $\pi$ braiding between a vison loop and either pointlike charge. The electron is the gauge-neutral composite $c = b^\dagger f$ — a gapped charge gap (chargon) and spin gap (BdG) with no symmetry breaking: exactly the "featureless-looking but topologically ordered" resolution of the filling-4 LSM constraint.
...
3. **Vison symmetry fractionalization:** a vison loop linking a 4a site braids with the unit gauge charge sitting there ($\pi$ phase). In Class II the background charges are additionally $C_{2z}$-odd, which twists the (projective) action of the screw on symmetric vison-loop configurations — charge and flux fractionalization classes are not independent, they must jointly reproduce the electron's linear quantum numbers. Measurable in principle through symmetry quantum numbers of flux-loop excitations encircling high-symmetry axes, or in the entanglement spectrum on $C_{2z}$-symmetric cuts.
